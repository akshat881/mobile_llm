...
public class FlutterLlamaPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
    ...
    private func loadModel(call: FlutterMethodCall, result: @escaping FlutterResult) {
        queue.async { [weak self] in
            ...
            // Use withCString to safely pass Swift String as const char* to C
...
// These functions are implemented in llama_cpp_bridge.mm
// IMPORTANT: Use UnsafePointer<CChar> (not String) because @_silgen_name
// bypasses automatic Swift-to-C bridging. Swift String has a different
// memory layout than const char* and causes EXC_BAD_ACCESS in strlen.
